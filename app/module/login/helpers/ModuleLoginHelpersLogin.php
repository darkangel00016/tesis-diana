...
class ModuleLoginHelpersLogin
{
    ...
    public static function forgot($twig)
    {
        $usuario = new ModuleUserEntityUsuario;
            ...
            if($usuario->findByField($email, "usuario")) {
                ...
                if($usuario->save()) {
                    $sentMail = CoreMail::sentMail(
                        ...
                        _("Olvido de clave"),
                        $twig->render("@ModuleApi/forgot.html.twig", array( "user" => $usuario, "code" => $code )),
                        $twig->render("@ModuleApi/forgot.txt.twig", array( "user" => $usuario, "code" => $code ))
                    );
                    if(!$sentMail["error"]) {
