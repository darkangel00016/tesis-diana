...
class CoreORM {
    /**
     * Working instance of CoreORM created earlier
     *
     * @var CoreORM
     */
    protected $db;
    ...
    private function join ($objectName, $key = null, $joinType = 'LEFT') {
        ...
        if (!$key)
            $key = $objectName . "id";
        $joinStr = CoreDb::$prefix . $this->dbTable . ".{$key} = " .
            CoreDb::$prefix . "{$joinObj->dbTable}.{$joinObj->primaryKey}";
